...
> **研究用途（参考性）**：本图谱可作为**供应链图分析、脆弱性建模与图神经网络（GNN）教学的参考性实验数据**（MIT 许可，尚非成熟基准）。
> 它以单一数据源 `data/apple_supply_chain.json` 为事实来源，纯 Python 标准库可复现重建；
> 但规模有限、属 AI 联网检索公开资料的二手整合与单点快照，使用前请务必注明数据口径与局限性
> （详见仓库「[分析方法局限性](https://github.com/Coolgiserz/apple-chain-graph#分析方法局限性)」），勿作一手事实或权威基准。
> 下文给出节点 / 关系字段字典，便于直接构造邻接矩阵或导入图数据库后导出边表。
