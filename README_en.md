...
- **2nd-hop downstream highlight**: selecting a supplier / component highlights its 2nd-hop downstream products, making "disruption hits N products" visible on the graph (e.g. Goertek's disruption reaches 46 products).
...
- **Edge `source` includes inference — it is not per-edge physical evidence**: `USES_COMPONENT` (Product → Component) edges are generated by applying a **product-line BOM template** (`PHONE_COMP` / `MAC_COMP` / `PAD_COMP` constants in `scripts/generate.py`), and their `source` is uniformly labelled `techinsights / ifixit`. That means the BOM *structure* draws on those two firms' industry teardown analysis — it does **not** mean every model was individually dismantled and verified, especially models released in 2025–2026. By contrast, `SUPPLIED_BY` (Component → Supplier) edges are hand-curated with per-edge `note` qualifiers and carry a higher evidence grade; even there, Chinese A-share filings are bound by confidentiality and typically disclose only "a major customer", so entries marked "customer not named" mean Apple supply is *inferred*, not confirmed.
- **Product attribute fields have no independent provenance**: `price_usd` / `release_date` / `soc` / `status` carry no source label; their convention is documented only as comments inside the generator (e.g. "official list price" vs "inferred from price-gap structure"). The data snapshot time is `meta.generated`; citing it across time requires re-checking Apple's live product pages.
