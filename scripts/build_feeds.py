...
输出到 data/feeds/{risk,valuation,sentiment}.json，每个文件遵循统一信封契约：
    {
      "meta": { "dataset", "version", "schema_ref", "generated", "valid_until", "sources" },
      "data": { ... 具体指标 ... }
    }
...
def envelope(dataset, data, generated, ttl, sources):
    valid = ""
    if generated:
        try:
...
def build_risk(ttl):
    ...
    return envelope("risk", data, generated, ttl, sources)


def build_valuation(ttl):
    ...
    return envelope("valuation", data, generated, ttl, sources)


def build_sentiment(ttl):
    ...
    if not os.path.exists(md_path):
        ...
        return envelope("sentiment", {"snapshot_date": today, "coverage": None, "markdown": ""}, today, ttl, [])
    ...
    return envelope("sentiment", data, snapshot, ttl, sources)
...
def main():
    ...
    write(args.out, "risk", build_risk(args.ttl_days))
    write(args.out, "valuation", build_valuation(args.ttl_days))
    write(args.out, "sentiment", build_sentiment(args.ttl_days))
    print("done. (ttl=%d days)" % args.ttl_days)
